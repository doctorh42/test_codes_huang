...
for metric in ["min", "max", "avg"]:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"./output5/{filename1}_{metric}_fitness_{num_generations}gens.png")
    plt.show()
...
for metric, values in zip(["min", "max", "avg"], [0, 1, 2]):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"./output5/{filename1}_{metric}_edge_length_{num_generations}gens.png")
    plt.show()
