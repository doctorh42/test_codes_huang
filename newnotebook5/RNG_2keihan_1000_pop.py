...
# 读取图和位置信息
filename1 = "2keihan2_pop_1000_rng"
read_graph, read_pos = fo.read_files(f"./RNG_net/{filename1}.net")
...
def plot_results(shortest_path_sums, total_edge_lengths, min_fitness_values, original_network_value,
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"./GA_P/{filename}_shortest_path_sum.png")
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"./GA_P/{filename}_total_edge_length.png")
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"./GA_P/{filename}_min_fitness.png")
    plt.show()
...
for future in as_completed(future_to_params):
    ...
    try:
        ...
        # 保存最终网络图像
        output_path = f"./output/{filename1}_ga.png"
        graph_draw(
            final_best_graph,
            pos=read_pos,
            edge_pen_width=2.0,
        ...
        # 保存最终网络为 .net 文件
        output_net_path = f"./GA_net/{filename1}_ga.net"
