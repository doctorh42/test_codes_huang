...
# 变异操作
def mutate(graph, mutation_rate, available_edges):
    if random.random() < mutation_rate:
        edges = list(graph.edges())
        if edges:  # Ensure there is at least one edge to remove
...
# 主遗传算法
def genetic_algorithm(graph, available_edges, pop_size, num_generations, mutation_rate, tournament_size):
    ...
    for generation in range(num_generations):
        ...
        for _ in range(pop_size // 2):
            ...
            mutate(child1, mutation_rate, available_edges)
            mutate(child2, mutation_rate, available_edges)
