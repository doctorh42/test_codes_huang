...
# 定义适应度函数
def evaluate(individual, read_graph, read_pos, edge_weights, original_shortest_path_sum, original_total_edge_length):
    # 解码个体为图
    ...
    if num_edges != read_graph.num_edges():
        return float('inf'),  # 不符合要求的个体适应度设为无穷大
    ...
    # 根据原始网络的比例动态计算权重
    w1 = original_total_edge_length / (original_shortest_path_sum + original_total_edge_length)
    w2 = 1 - w1

    # 计算适应度值
    fitness_value = w1 * total_distance + w2 * total_edge_length
